...
#Function for converting dataframe of screped data to list of list.
#This function returns list of list where first position in each index is product name and second position is list of bought products.
def convert_to_list(in_df):
    rows = []
    rows_product_name = []
    for i in in_df.iterrows():
        each_row = []
        product_name = []
        each_row = [i[1][1], i[1][2], i[1][3], i[1][4], i[1][5], i[1][6]]
        product_name.append(i[1][0])
        rows_product_name.append(product_name)
        rows.append(each_row)
    for i in range (len(rows)):
        rows_product_name[i].append(rows[i])
    return rows_product_name
